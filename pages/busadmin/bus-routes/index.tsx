...
function BusListing() {
  ...
  const columns: ColumnsType<DataType> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <i className="fa fa-pen text-primary cursor-pointer" style={{ fontSize: "20px" }} onClick={() => Router.push(`/busadmin/bus-routes/edit/${record.id}`)} />
    ...
    <BusadminLayout title="Bus Routes">
      {
        !data && !error ?
          ...
          <Table
            ...
            columns={columns}
            dataSource={data?.data?.map((route: any) => ({ ...route, description: route.description.slice(0, 15) + '...' }))}
          />
      }
    </BusadminLayout>
  )
