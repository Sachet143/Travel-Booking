...
function BusRoutes({ submitHandler, formMethods }: any) {
  ...
  const { append: appendBoards, remove: removeBoards, fields: fieldsBoards } = useFieldArray({ control, name: "boards" } as any);
  const { append: appendDrops, remove: removeDrops, fields: fieldsDrops } = useFieldArray({ control, name: "drops" } as any);
        ...
        <div className='row'>
          ...
          <div className="col-md-12 mb-3">
            <div className='d-flex gap-3'>
              <h4>Board</h4>
              <Button size='small' type='primary' onClick={() => appendBoards({ location: "", time: "" })}>+ Add</Button>
            </div>
            {
              fieldsBoards.map((field, i) =>
                <div className='row' key={field.id}>
                        ...
                        render={({ field: { onChange, value } }) => (
                          <>
                            ...
                            {errors?.boards?.[i]?.location?.message && (
                              <div className="text-danger">{errors?.boards?.[i]?.location.message + ""}</div>
                            )}
                          </>
                        )}
                      />
                  ...
                  <div className="col-md-4 form-group mb-3">
                    ...
                    <input
                      {...register(`boards.${i}.time`, { required: "Time is required!" })}
                      aria-invalid={!!errors?.boards?.[i]?.time?.message}
                      className="form-control"
                      placeholder="Enter Location"
                    />
                    {errors?.boards?.[i]?.time?.message && (
                      <div className="text-danger">{errors?.boards?.[i]?.time.message + ""}</div>
                    )}
                  </div>
                  <div className="col-md-4 form-group mb-3 align-self-center">
                    <Button size='small' danger onClick={() => fieldsBoards.length > 1 && removeBoards(i)}>Delete</Button>
                  </div>
                </div>
              )
            }
          ...
          <div className="col-md-12 mb-3">
            <div className='d-flex gap-3'>
              <h4>Drop</h4>
              <Button size='small' type='primary' onClick={() => appendDrops({ location: "", time: "", price: "" })}>+ Add</Button>
            </div>
            {
              fieldsDrops.map((field, i) =>
                <div className='row' key={field.id}>
                        ...
                        render={({ field: { onChange, value } }) => (
                          <>
                            ...
                            {errors?.drops?.[i]?.location?.message && (
                              <div className="text-danger">{errors?.drops?.[i]?.location?.message + ""}</div>
                            )}
                          </>
                        )}
                      />
                  ...
                  <div className="col-md-3 form-group mb-3">
                    <label className="form-label">Time (In Minutes)</label>
                    <input
                      {...register(`drops.${i}.time`, { required: "Time is required!" })}
                      aria-invalid={!!errors?.drops?.[i]?.time?.message}
                      className="form-control"
                      placeholder="Enter Time"
                    />
                    {errors?.drops?.[i]?.time?.message && (
                      <div className="text-danger">{errors?.drops?.[i]?.time.message + ""}</div>
                    )}
                  </div>
                  <div className="col-md-3 form-group mb-3">
                    <label className="form-label">Price</label>
                    <input
                      {...register(`drops.${i}.price`, { required: "Price is required!" })}
                      aria-invalid={!!errors?.drops?.[i]?.time?.message}
                      className="form-control"
                  ...
                  <div className="col-md-3 form-group mb-3 align-self-center">
                    <Button size='small' danger onClick={() => fieldsDrops.length > 1 && removeDrops(i)}>Delete</Button>
                  </div>
                </div>
              )
            }
